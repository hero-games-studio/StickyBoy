...
public class trajectoryScript : MonoBehaviour
{
    ...
    [SerializeField]
    public GameObject[] stickyPoints;
    ...
    void Start()
    {
        ...
        trajectoryDots.transform.localScale = new Vector3(initialDotSize, initialDotSize, trajectoryDots.transform.localScale.z); 
                                                                    ...
                                                                    //Initial size of trajectoryDots is applied
    ...
    internal void checkStickedPoints()
    {
        stickedObject = null;
        foreach (GameObject st in stickyPoints)
        {
            if (st.GetComponent<Sticky>().sticked)
            {
                if (stickedObject == null)
                {
                    stickedObject = st.GetComponent<Sticky>().stickedObject.gameObject;
                }
                else
                {
                    if (stickedObject != st.GetComponent<Sticky>().stickedObject.gameObject)
                    {
                        st.GetComponent<Sticky>().releaseJoint();
                    }
                }
            }
        }
    ...
    void Update()
    {
        ...
        //Used to determine if the finger/cursor is on the Ball Click Area
        ...
        if ((ballRB.velocity.x * ballRB.velocity.x) + (ballRB.velocity.y * ballRB.velocity.y) <= speedLimit)
    ...
    internal void readyToJump()
    {
        if ((ballRB.velocity.x <= 0.2f && ballRB.velocity.y <= 0.2f) || (canJump == true || doubleJump == true))
        {   //If player has activated a shot										//when you press down
            ...
            if (grabWhileMoving == true && (canJump == true || doubleJump == true))
    ...
    internal void jumpNow()
    {
        //If the player lets go...
        ballIsClicked2 = false;
        //Aiming is no longer happening

        playerScript.DidGrapObject();
